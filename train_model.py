...
# 学习参数
checkpoint =None# "./results/deepWBNet/checkpoint_deepWBNet1000.pth"  # 预训练模型路径，如果不存在则为None
batch_size = 25  # 批大小
...
def main():
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for i, (lr_imgs, hr_imgs) in enumerate(train_loader):
            ...
            '''
            if i == (n_iter - 2):
                writer.add_image('deepWBNet/epoch_' + str(epoch) + '_1',
                                 make_grid(lr_imgs[:4, :3, :, :].cpu(), nrow=4, normalize=True), epoch)
                writer.add_image('deepWBNet/epoch_' + str(epoch) + '_2',
                                 make_grid(sr_imgs[:4, :3, :, :].cpu(), nrow=4, normalize=True), epoch)
                writer.add_image('deepWBNet/epoch_' + str(epoch) + '_3',
                                 make_grid(hr_imgs[:4, :3, :, :].cpu(), nrow=4, normalize=True), epoch)
            '''

            # 打印结果
            # print("第 "+str(i)+ " 个batch训练结束")
        print("-------epoch=" + str(epoch) + "/" + str(epochs) + ",loss_epoch=" + str(
            loss_epoch.avg) + "------------------")
        #Loss0 = np.array(loss_epoch.avg)
        #np.save('./data/results/loss_unet/epoch_{}'.format(epoch), Loss0)
        # 手动释放内存
        del lr_imgs, hr_imgs, sr_imgs

        # 监控损失值变化
        #writer.add_scalar('deepWBNet/MSE_Loss', loss_epoch.val, epoch)
        # 保存预训练模型
        if epoch %100==0:
            torch.save({
            'epoch': epoch,
        ...
        }, './results/upper_gi_hyper_kvasir_all/deepWBNet/checkpoint_deepWBNet'  +str(epoch) + '.pth')
