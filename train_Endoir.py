...
epochs = 10000  # 迭代轮数
checkpoint ="./results/wbgan/checkpoint_wbgan_600.pth"#"./results/unetgan/checkpoint_unetgan_2000.pth"  # SRGAN预训练模型, 如果没有则填None
workers = 4  # 加载数据线程数量
vgg19_i = 5  # VGG19网络第i个池化层
vgg19_j = 4  # VGG19网络第j个卷积层
beta = 1e-3  # 判别损失乘子
lr = 1e-4  # 学习率
alpha = 0.8  # 内容损失和颜色损失乘子
...
def color_loss(sr_image, hr_image):
    """
    :param sr_image:
    :param hr_image:
    :return: 两者之间的颜色损失
    """
    c0 = cosine_similarity(sr_image[:, :, :, 0].reshape(1, -1), hr_image[:, :, :, 0].reshape(1, -1))
    c1 = cosine_similarity(sr_image[:, :, :, 1].reshape(1, -1), hr_image[:, :, :, 1].reshape(1, -1))
    c2 = cosine_similarity(sr_image[:, :, :, 2].reshape(1, -1), hr_image[:, :, :, 2].reshape(1, -1))
    color_loss = np.mean([c0, c1, c2])
    return color_loss
...
def main():
    ...
    # srresnetcheckpoint = torch.load(srresnet_checkpoint,map_location=torch.device('cpu'))
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for i, (lr_imgs, hr_imgs) in enumerate(train_loader):
            ...
            # 计算颜色损失
            # color_loss = color_loss(sr_imgs, hr_imgs)
        ...
        print("-------epoch=" + str(epoch) + "/" + str(epochs) + ",losses_c=" + str(losses_c.avg) + ",losses_a=" + str(
            losses_a.avg) + ",losses_d=" + str(losses_d.avg) + "------------------")
        # Loss0 = np.array([losses_c.avg,losses_a.avg,losses_d.avg])
        # np.save('./data/results/loss_srgan/epoch_{}'.format(epoch),Loss0)
        # 手动释放内存
        ...
        if epoch %100==0:
            ...
            }, './results/wbgan/checkpoint_wbgan_' + str(epoch) + '.pth')
